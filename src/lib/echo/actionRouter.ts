...
export function routeActions(
  ...
  if (intent.kind === "CONTINUE" && target)
    return [{
      ...base,
      id: `${actionInstanceId}:continue-current-thread`,
  ...
  if (intent.kind === "CORRECTION" && target)
    return [{
      ...base,
      id: `${actionInstanceId}:update-conversation-focus`,
  ...
  if (intent.kind === "ACTION_REQUEST" && target)
    return [{
      ...base,
      id: `${actionInstanceId}:review-request`,
      kind: "REVIEW_REQUEST",
  ...
  if (intent.kind === "INFORMATION_REQUEST")
    return [{
      ...base,
      id: `${actionInstanceId}:answer-in-conversation`,
